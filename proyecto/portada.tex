\begin{titlepage}
  \thispagestyle{empty}
  %--------------- COMANDO LINEA----------------->
  \newcommand{\linea}{\rule{\linewidth}{0.5mm}}                 
  \center
  %<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

  %Add logos
  \includegraphics[width=0.24\textwidth]{unam_logo.png} \hspace{1.5cm}
  \includegraphics[width=0.25\textwidth]{fc_logo.png} \\[1cm]
  \textbf{\Large UNIVERSIDAD NACIONAL AUTÓNOMA}\\[3mm]
  \textbf{\Large DE MÉXICO} \\[6mm]
  \textit{\Large Facultad de Ciencias}

  \vfill

  %---------------TÍTULO----------------->
  \linea
  \vfill \vspace{1cm}
  \textbf{\Large Reconocimiento de Patrones y Aprendizaje Automatizado}\\[0.7cm]
  \textbf{\Large Proyecto Final}\\[0.3cm]
  \linea \\
  \vfill
  \vspace{0.7cm}
  %Title of the Research
  \textbf{\Large  Análisis de Cardiotocografía Fetal\\ mediante Clustering}\\[6mm]
  %<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

  %Team
  \vfill
  \textit{\small Presenta:}\\
  \textbf{\large \textbf{Hernández Zavala Ana Sofía}}\\[2mm]
  \textbf{\large \textbf{Ramírez Juárez María Fernanda}}\\[2mm]
  \textbf{\large \textbf{Rojo Peña Manuel Ianluck}}\\[2mm]

  %Profesor y Grupo
  \textit{\small Profesor:}\\
  \textbf{Sergio Rodolfo Cruz Gómez}\\[2mm]
  \textit{\small Ayudantes:}\\
  \textbf{Gustavo Adolfo Garduño Martínez}\\[1mm]
  \textbf{Julio César Misael Monroy Gónzalez}\\[1mm]
  \small Grupo: 7142, 2026-2\\[2mm]
  \vfill

  %Fecha or Date
  \textit{\small Fecha de entrega:}\\
         {\large 24 de mayo, 2026}
         
\end{titlepage}
